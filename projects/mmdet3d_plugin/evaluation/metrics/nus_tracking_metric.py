...
from projects.mmdet3d_plugin.datasets import NuScenesForecastingBox
from projects.mmdet3d_plugin.distr.custom_collect import \
    custom_collect_results
...
class NuScenesTrackingMetric(NuScenesMetric):
    ...
    def _format_lidar_bbox(
        ...
        for i, det in enumerate(mmengine.track_iter_progress(results)):
            ...
            for i, box in enumerate(boxes):
                ...
                det_annos.append(nusc_anno)
                # filter out the classes not for tracking
                if (name in NuScenesTrackingMetric.TRACKING_CLASSES):
                    track_annos.append(nusc_anno)
            nusc_det_annos[sample_token] = det_annos
            nusc_track_annos[sample_token] = track_annos

        nusc_submissions = {
            "meta": self.modality,
            "results": nusc_det_annos,
        }
        mmengine.mkdir_or_exist(jsonfile_prefix)
        det_res_path = osp.join(jsonfile_prefix, "results_nusc_detection.json")
        print_log(f"Results writes to {det_res_path}")
        mmengine.dump(nusc_submissions, det_res_path)

        nusc_submissions = {
            "meta": self.modality,
            "results": nusc_track_annos,
        }
        track_res_path = osp.join(jsonfile_prefix, "results_nusc_tracking.json")
        print_log(f"Results writes to {track_res_path}")
        mmengine.dump(nusc_submissions, track_res_path)

        return det_res_path, track_res_path
    ...
    def _evaluate_single(
        ...
        Args:
            result_paths (str): Tuple of the result files
        ...
        det_path, track_path = result_paths
        output_dir = osp.join(*osp.split(det_path)[:-1])
        ...
        try:
            ...
            for key in self.KEYS:
                ...
                for name in self.TRACKING_CLASSES:
                    detail[f"{metric_prefix}/{name}_{key}"] = metrics["label_metrics"][
                        key
                    ][name]
        except Exception as e:
            # log the error and continue
            print_log(f"Error occurs when evaluating {track_path}: {e}")

        try:
            # evaluate detection metrics
            det_metrics = super()._evaluate_single(det_path, classes, result_name)
            detail.update(det_metrics)
        except Exception as e:
            print_log(f"Error occurs when evaluating {det_path}: {e}")
    ...
    def evaluate(self, size: int) -> dict:
        ...
        if is_main_process():
            ...
            if self.prefix:
                _metrics = {"/".join((self.prefix, k))
                                     : v for k, v in _metrics.items()}
            metrics = [_metrics]
        else:
...
def lidar_nusc_box_to_global(info, boxes, classes, eval_configs):
    ...
    for box in boxes:
        ...
        box.rotate(pyquaternion.Quaternion(

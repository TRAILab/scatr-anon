...
class LiftSplatShoot(nn.Module):
    ...
    def get_geometry(self, rots, trans, post_rots=None, post_trans=None, extra_rots=None, extra_trans=None, img_metas=None):
        ...
        if 'img_aug_matrix' in img_metas[0]:
            post_rots = torch.stack(
                [i['img_aug_matrix'][..., :3, :3] for i in img_metas], dim=0).to(rots)
        ...
        for b in range(B):
            points[b] = apply_3d_transformation(
                points[b].view(-1, 3), 'LIDAR', img_metas[b], reverse=False).view(*point_shape)

        if extra_rots is not None or extra_trans is not None:
            if extra_rots is not None:
                points = extra_rots.view(B, N, 1, 1, 1, 3, 3).matmul(
    ...
    def bev_pool(self, geom_feats, x):
        ...
        # flatten indices
        geom_feats = (
            (geom_feats - (self.bx - self.dx / 2.0)) / self.dx).long()
    ...
    def get_voxels(self, x, rots=None, trans=None, post_rots=None, post_trans=None, extra_rots=None, extra_trans=None, img_metas=None):
        geom = self.get_geometry(
            rots, trans, post_rots, post_trans, extra_rots, extra_trans, img_metas=img_metas)
    ...
    def forward(self, x, rots, trans, lidar2img_rt=None, img_metas=None, post_rots=None, post_trans=None, extra_rots=None, extra_trans=None):
        x, depth = self.get_voxels(x, rots, trans, post_rots, post_trans,
                                   extra_rots, extra_trans, img_metas=img_metas)  # [B, C, H, W, L]
        bev = self.s2c(x)
